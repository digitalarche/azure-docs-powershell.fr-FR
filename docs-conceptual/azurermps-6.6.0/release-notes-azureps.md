...
* Mise à jour de la bibliothèque Common.Strategy pour être en mesure de valider que la configuration actuelle pour une ressource est compatible avec la ressource cible. Par défaut, ce sont toujours de vraies ressources individuelles et elles remplacent la valeur par défaut.
* Ajout des types ps1xml à Common.Storage

#### <a name="azurestorage"></a>Azure.Storage
* Le support obtient le contexte de stockage à partir de DefaulfProfile
* Ajoute de Ps1XmlAttribute aux propriétés de types de sortie des cmdlets.
