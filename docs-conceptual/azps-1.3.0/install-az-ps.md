...
Azure PowerShell fonctionne avec PowerShell 5.1.x ou version supérieure sur Windows ou sur PowerShell 6.x sur n’importe quelle plateforme.
...
Si vous utilisez PowerShell 5.x sur Windows, vous devez également avoir installé .NET Framework 4.7.2. Pour obtenir des instructions sur la mise à jour ou l’installation d’une nouvelle version de .NET Framework, consultez le [guide d’installation de .NET Framework](/dotnet/framework/install).
