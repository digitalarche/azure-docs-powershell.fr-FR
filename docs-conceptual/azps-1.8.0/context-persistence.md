...
L’applet de commande [Set-AzContext](/powershell/module/Az.Accounts/Set-AzContext?view=azps-1.8.0) est utilisée à la fois pour créer des contextes Azure et pour les définir comme contexte actif.
...
`Set-AzContext` et [Select-AzContext](/powershell/module/az.accounts/set-azcontext?view=azps-1.8.0) peuvent toutes deux être utilisées pour changer le contexte Azure actif. Comme décrit dans [Créer un contexte Azure](#create-a-new-azure-context-from-subscription-information), `Set-AzContext` crée un contexte Azure pour un abonnement s’il n’en existe pas, puis utilise ce contexte comme contexte actif.
