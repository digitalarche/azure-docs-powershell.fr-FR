...
Les modules Azure PowerShell pour .NET Core sont en cours de développement. Ces modules ne fournissent pas l’ensemble des cmdlets qui sont disponibles pour la version Windows des modules. Les fonctions suivantes sont implémentées dans les modules AzureRM.Netcore :
...
* Cmdlets de plan de gestion pour les services Azure utilisant des interfaces de modèle de déploiement classiques et Resource Manager.
  * Machine virtuelle
