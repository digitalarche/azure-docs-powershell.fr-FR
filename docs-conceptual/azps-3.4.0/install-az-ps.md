...
Si vous avez utilisé Install-Module, vous devez utiliser [Update-Module](/powershell/module/powershellget/update-module) pour obtenir la version la plus récente. Si vous avez utilisé le package MSI, vous devez télécharger et installer le nouveau MSI afin de pouvoir effectuer une mise à jour. Si vous rencontrez des problèmes de mise à jour en utilisant le package de PowershellGet, vous devez __réinstaller__, plutôt que juste __mettre à jour__. Cette opération s’effectue de la même façon que l’installation, mais il peut être nécessaire d’ajouter l’argument `-Force` :
