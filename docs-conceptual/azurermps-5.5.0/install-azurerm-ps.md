...
Si vous souhaitez utiliser Azure PowerShell sur macOS ou Linux, consultez l’article suivant :  
[Installer et configurer Azure PowerShell sur macOS et Linux](install-azurermps-maclinux.md).
...
```
En outre, vous pouvez mettre à jour PowerShellGet avec la commande suivante :
```powershell
...
|Je veux effectuer la mise à niveau vers PowerShell 5|<ol><li>[Installer la dernière version de WMF](https://www.microsoft.com/en-us/download/details.aspx?id=54616)</li><li>Exécutez la commande suivante :<br/>```Install-Module PowerShellGet -Force```</li></ol>|
|Mon ordinateur exécute une version de Windows avec PowerShell 3 ou 4|<ol><il>[Obtenir les modules PackageManagement](http://go.microsoft.com/fwlink/?LinkID=746217)</il><li>Exécutez la commande suivante :<br/>```Install-Module PowerShellGet -Force```</li></ol>|
